...
(function ($) {
    ...
    var options = {
            ...
            ruleScores: {
                ...
                wordIsOrderlyString: -100,
				wordIsSameString: -100
            },
            rules: {
                wordNotEmail: true,
                wordLength: true,
                ...
                wordIsOrderlyString:true,
				wordIsSameString:true
            },
            validationRules: {
                ...
                wordIsOrderlyString:function(options,word, score){
            		var s=true;
            		for(var p=0; p<word.length-1;p++){
            				var r=word.charCodeAt(p);
            				var q=word.charCodeAt(p+1);
            				if(isNaN(q)||isNaN(r)){
            						continue
            				}else{
            					if(Math.abs(r-q)!=1||r==47||r==57||r==64||r==90||r==96||r>122){
            						s=false;
            						break
            					}
            				}
					...
					if(s)	
					{				
						options.isConsectiveChars = true;
						return score;
					}
					else
					{
						options.isConsectiveChars = false;
						return 0;
					}
                },
				wordIsSameString : function(options, word, score){
					var s=true;
					for(var p=0;p<word.length-1;p++){
						var r=word.charCodeAt(p);
						var q=word.charCodeAt(p+1);
					...
					if(s)	
					{		
						options.isSameString = true;				
						return score;
					}
					else
					{
						options.isSameString = false;	
						return 0;
					}
				}
            }
        ...
        setProgressBar = function ($el, score) {
            ...
            if (score < options.scores[0] || options.isSameString || options.isConsectiveChars) {
